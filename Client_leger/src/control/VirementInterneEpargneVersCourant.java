...
import objetMetier.MetierVirementInterneEpargneVersCourant;

/**
 * Servlet implementation class VirementInterneEpargneVersCourant
 */
@WebServlet("/VirementInterneEpargneVersCourant")
public class VirementInterneEpargneVersCourant extends HttpServlet {
	private static final long serialVersionUID = 1L;
	public static final String ATT_ERREURS = "erreurs";
	public static final String ATT_RESULTAT = "resultat";

	public VirementInterneEpargneVersCourant() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		MetierVirementInterneEpargneVersCourant alimenter = new MetierVirementInterneEpargneVersCourant();
		...
		if (idEpargne != 0) {
			if (idTitre != 0) {
				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneEpargneVersCourantAvecEpargneAvecTitre.jsp")
						.forward(request, response); // Avec Epargne + avec titre
			} else {
				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneEpargneVersCourantAvecEpargneSansTitre.jsp").forward(request,
						response); //Avec Epargne + Sans titre
			}
		} else {
			if (idTitre != 0) {
				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneEpargneVersCourantSansEpargneAvecTitre.jsp")
						.forward(request, response); // Sans Epargne + avec titre
			} else {

				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneEpargneVersCourantSansEpargneSansTitre.jsp").forward(request,
