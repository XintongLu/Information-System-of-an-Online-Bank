 ...
 */
@WebServlet("/VirementInterneTitreVersCourant")
public class VirementInterneTitreVersCourant extends HttpServlet {
	private static final long serialVersionUID = 1L;
	public static final String ATT_ERREURS = "erreurs";
	public static final String ATT_RESULTAT = "resultat";

	public VirementInterneTitreVersCourant() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		MetierVirementInterneTitreVersCourant alimenter = new MetierVirementInterneTitreVersCourant();
		...
		if (idEpargne != 0) {
			if (idTitre != 0) {
				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneTitreVersCourantAvecEpargneAvecTitre.jsp")
						.forward(request, response); // Avec Epargne + avec titre
			} else {
				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneTitreVersCourantAvecEpargneSansTitre.jsp").forward(request,
						response); //Avec Epargne + Sans titre
			}
		} else {
			if (idTitre != 0) {
				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneTitreVersCourantSansEpargneAvecTitre.jsp")
						.forward(request, response); // Sans Epargne + avec titre
			} else {

				this.getServletContext().getRequestDispatcher("/VirementInterne/VirementInterneTitreVersCourantSansEpargneSansTitre.jsp").forward(request,
